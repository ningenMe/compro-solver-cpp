...
/**
 * @url https://tlx.toki.id/contests/troc-16/problems/C
 * @est 400
 */ 
int main() {
    cin.tie(0);ios::sync_with_stdio(false);
    int N,M; cin >> N >> M;
    auto dp = multivector(N+1,2,2,modint(0));
    dp[0][0][0]=1;
    for(int i=1;i<=N;++i) {
        //i番目がMじゃない かつ Mが未出
        dp[i][1][1] += dp[i-1][0][0];
        dp[i][0][0] += dp[i-1][0][0]*(M-1);
        //i番目がM
        dp[i][0][1] += dp[i-1][1][1]*(M-1);
        //i番目がMじゃない かつ Mが既出
        dp[i][1][1] += dp[i-1][0][1];
        dp[i][0][1] += dp[i-1][0][1]*(M-1);
    }
