...
template<class T> struct FormalPowerSeries : public vector<T> {
    ...
    inline vector<RuntimeModInt<mod>> convolution(const vector<RuntimeModInt<mod>>& g,const vector<RuntimeModInt<mod>>& h){return convolution_arbitrarymod(g,h);}
...
int main(){
    ...
    a *= b;
    for(int i = 0; i+1 < N+M; ++i) cout << a[i] << " "; cout << endl;
    return 0;
}
