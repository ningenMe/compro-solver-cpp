...
template<class T> struct NodeMaxPointUpdate {
	using TypeNode = T;
	inline static constexpr TypeNode unit_node = {-LOWINF,-LOWINF,-LOWINF,-LOWINF};
	inline static constexpr TypeNode func_node(TypeNode l,TypeNode r){
        array<ll,4> ret = {
            max(l[0]+max(r[3],0LL),max(l[2],0LL)+r[0]),
            max(l[1]+max(r[2],0LL),max(l[3],0LL)+r[1]),
            max(l[0]+r[1],max(l[2],0LL)+max(r[2],0LL)),
            max(l[1]+r[0],max(l[3],0LL)+max(r[3],0LL))
        };
        return ret;}
...
ll maxi(const array<ll,4> ar) {
	ll ret = -LOWINF;
	for(int i = 0; i < 4; ++i) chmax(ret,ar[i]);
	return ret;
}
...
void solve() {
	...
	SegmentTree<NodeMaxPointUpdate<array<ll,4>>> seg(A);
	// seg.print();
	{
		cout << maxi(seg.get(0,N)) << "\n";
	}
	while(Q--) {
		int l,r; cin >> l >> r;
		l--,r--;
