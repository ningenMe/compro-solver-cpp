...
inline ll inversion_num(const vector<unsigned int>& a) {
    int N = a.size();
...
int main() {
    ...
    ll cnt = inversion_num(A);
    for(int i = 30; 0 <= i; --i) {
        ll target = prefix + (1LL<<i);
        for(int j=0;j<N;++j) B[j]=target^A[j];
        ll cnt_t = inversion_num(B);
        if(cnt > cnt_t) {
            cnt = cnt_t;
            prefix = target;
        }
