...
template<class T> struct NodeMaxPointUpdate {
    ...
    inline static constexpr bool func_check(TypeNode nodeVal,TypeNode var){return nodeVal >= var;}
};
...
int main() {
    ...
    int N; cin >> N;
    vector<int> idx(N+1);
    SegmentTree<NodeMaxPointUpdate<int>> seg(2*N,0),seg2(2*N,0);
    for(int i=0;i<2*N;++i) {
        char c; cin >> c;
        if(c=='+') {
            seg.update(i,2*N);
    ...
    for(int i=1;i<=N;++i) {
        int r = idx[i];
        corner(seg.get(0,r) < 2*N,"NO");
        int l = seg.suffix_binary_search(-1,r,2*N); 
        corner(seg2.get(l,r)>i,"NO");
        seg.update(l,i);
        a[l]=i;
    }
    vector<int> ans;
    for(int i=0;i<2*N;++i) if(a[i]) ans.push_back(a[i]);
    cout << "YES" << endl;
