...
int main() {
	...
	vector<pair<int,set<int>>> dp(N);
	vector<ll> ans(N);
	for(auto pa:tree.order) {
		mp[pa][A[pa]]=1;
		dp[pa]={1,set<int>()};
		dp[pa].second.insert(A[pa]);
		ans[pa]=A[pa];

		for(auto& e:tree.child[pa]) {			
			int ch=e.first;
			if(dp[pa].first < dp[ch].first) {
				swap(dp[pa],dp[ch]);
				ans[pa]=ans[ch];
			}			
			if(dp[pa].first == dp[ch].first) {
				if(dp[pa].second.size() < dp[ch].second.size()) {
					swap(dp[pa],dp[ch]);
					ans[pa]=ans[ch];
				}
				for(auto& s:dp[ch].second) {
					if(!dp[pa].second.count(s)) {
						ans[pa] += s;
						dp[pa].second.insert(s);
					}
				}
			}

			if(mp[pa].size() < mp[ch].size()) {
				swap(mp[pa],mp[ch]);
			}
			for(auto& p:mp[ch]) {
				int s = p.first;
				mp[pa][s] += p.second;
				if(mp[pa][s] < dp[pa].first) continue;
				if(mp[pa][s] == dp[pa].first) {
					if(!dp[pa].second.count(s)) {
						ans[pa] += s;
						dp[pa].second.insert(s);
					}
