...
int main() {
    int N; cin >> N;
    vector<ll> A(N+2,1e10),B(N+2,1e10);
    for(int i = 1; i <= N; ++i) cin >> A[i];
    for(int i = 1; i <= N; ++i) cin >> B[i];
    DisjointSparseTable<nodeSum<ll>> dst(A);
    vector<ll> L(N+2),R(N+2);
    for(int i = 1; i <= N; ++i) {
        int ok = N+1,ng = i-1,md;
        while (ok-ng>1) {
            md = (ok+ng)/2;
            (dst.get(i,md+1) >= B[i]?ok:ng)=md;
        }
        R[i] = ok;
    }
    for(int i = 1; i <= N; ++i) {
        ...
        while (ok-ng>1) {
            md = (ok+ng)/2;
            (dst.get(md,i+1) <= B[i]?ok:ng)=md;
        }
        L[i] = (i<ok?-1:ok);
    }
    priority_queue<pair<int,int>> pq;
    for(int i = 1; i <= N; ++i) if(L[i]!=-1) pq.push({L[i],i});
    SegmentTree<nodeSumPointAdd<ll>> Seg(N+2,0);
    ll ans = 0;
    for(int i = N; 1 <= i; --i) {
        if(L[i]!=-1) Seg.update(i,1);
        ans += Seg.get(R[i],N+1);
        while(pq.size()){
            auto p = pq.top();
            if(i <= p.first){
                pq.pop();
                Seg.update(p.second,-1);
            }
            else break;
        }
    }
