...
int main() {
	...
	SegmentTree<nodeSumPointAdd<ll>> seg(N);
	for(int i = 1; i < N; ++i) {
		int j = tree.hldorder[i];
		ll w = tree.parent[i].second;
		seg.update(j,w);
	}
    int Q; cin >> Q;
	vector<ll> ans(Q);
    for(int q = 0; q < Q; ++q) {
		...
		size_t lca = tree.hldLca(x[0],x[1]);
		for(int i = 2; i < k; ++i) lca = tree.hldLca(lca,x[i]);
		set<int> st;
		//{self hldorder,self}
		priority_queue<pair<int,int>> pq;
		for(int i = 0; i < k; ++i) pq.push({tree.hldorder[x[i]],x[i]});
		ll sum = -seg.get(tree.hldorder[lca],tree.hldorder[lca]+1);
		while(pq.size()){
			auto p = pq.top();
			int u = p.second;
			int h = tree.head[u];
			...
			st.insert(h);
			int pa = tree.parent[h].first;
			if(h != tree.head[lca]) pq.push({tree.hldorder[pa],pa});

			int l = max(tree.hldorder[lca],tree.hldorder[h]);
			int r = tree.hldorder[u];
			sum += seg.get(l,r+1);
		}
		ans[q] = sum;
	}
