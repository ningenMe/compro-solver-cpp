...
template <class T>ostream &operator<<(ostream &o, const array<T,2>&obj) {o << "{"; for (int i = 0; i < (int)obj.size(); ++i)o << (i > 0 ? ", " : "") << obj[i]; o << "}"; return o;}
...
template<class Operator> class LazySegmentTree {
	...
	void propagate(int k) {
		...
		if(k < length) lazy[2*k+0] = Operator::func_lazy(lazy[2*k+0],lazy[k],range[k].first,range[k].second);
		if(k < length) lazy[2*k+1] = Operator::func_lazy(lazy[2*k+1],lazy[k],range[k].first,range[k].second);
...
public:
	...
	void update(int a, int b, TypeLazy x) {
		...
		for(r++; l < r; l >>=1, r >>=1) {
			if(l&1) lazy[l] = Operator::func_lazy(lazy[l],x,range[l].first,range[l].second), propagate(l),l++;
			if(r&1) --r,lazy[r] = Operator::func_lazy(lazy[r],x,range[r].first,range[r].second), propagate(r);
		}
...
template<class T, class U> struct NodeSumRangeManyQuery {
	...
	inline static constexpr TypeLazy unit_lazy = {1,0,0};
	inline static constexpr TypeNode func_node(TypeNode l,TypeNode r){
        return {l[0]+r[0],l[1]+r[1]};
    }
	inline static constexpr TypeLazy func_lazy(TypeLazy pre_lazy,TypeLazy cur_lazy,int l, int r){
        pre_lazy[0] = cur_lazy[0]*pre_lazy[0];
        pre_lazy[1] = cur_lazy[0]*pre_lazy[1]+cur_lazy[1];
        pre_lazy[2] = cur_lazy[0]*pre_lazy[2]+cur_lazy[2];
        return pre_lazy;
    }
	inline static constexpr TypeNode func_merge(TypeNode node,TypeLazy lazy,int l, int r){
        return {lazy[0]*node[0]+lazy[1]*(r-l)+lazy[2]*node[1],node[1]};
    }
	inline static constexpr bool func_check(TypeNode nodeVal,TypeNode var){return var <= nodeVal;}
	// LazySegmentTree<NodeSumRangeUpdate<ll,ll>> Seg(N,0);
};

using no = array<modint,2>;
using la = array<modint,3>;
int main() {
    ...
    int N,Q; cin >> N >> Q;
    vector<no> a(N);
    for(int i=0;i<N;++i) a[i]={0,F[i]};
    LazySegmentTree<NodeSumRangeManyQuery<no,la>> seg(a);
    while(Q--) {
        int q,l,r,k; cin >> q >> l >> r >> k;
        r++;
        if(q==0) {
            cout << seg.get(l,r)[0]*k << endl;
            continue;
        }
        la query={0,0,0};
        if(q==1) query[1]=k;
        if(q==2) query[0]=1,query[1]=k;
        if(q==3) query[0]=k;
        if(q==4) query[0]=1,query[2]=k;
        seg.update(l,r,query);
    }
    return 0;
}
