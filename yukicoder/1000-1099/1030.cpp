...
template<class typeDist> struct treeOperator{
	...
	typeDist unitDist = 0;
	typeDist unitLca = 0;
	typeDist funcDist(const typeDist& parent,const typeDist& w){return max(parent,w);}
...
Tree<treeOperator<ll>> tree(100000);
...
//一点更新 区間GCD
template<class typeNode> struct nodeLCAPointUpdate {
	typeNode unitNode = -1;
	typeNode funcNode(typeNode l,typeNode r){
        if(l==-1) return r;
        else if(r == -1) return l;
        else return tree.lca(l,r).first;
    }
...
int main() {
    ...
    SegmentTree<nodeLCAPointUpdate<int>> seg(A);
    for(int i = 0; i < Q; ++i){
        int q; cin >> q;
        if(q==1){
            int x,y; cin >> x >> y;
