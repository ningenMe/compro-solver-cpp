...
int main() {
    ...
    string S; read(S);
    int M = 2*N+1;
    vector<pair<Mint,Mint>> dp(M);
    int BASE = N;
    dp[BASE]={1,0};

    for(int i=0;i<2*N;++i) {
        char c = S[i];
        vector<pair<Mint,Mint>> tp(M, {0,0});

        if(c=='+' || c=='?') {
            for(int j=-N; j<=N; ++j) {
                int key = BASE + j;
                if(j + 1 > N) continue;
                auto [cnt1,sum1] = dp[key];
                auto [cnt2,sum2] = tp[key+1];
                tp[key+1] = {cnt1+cnt2, cnt1*abs(j)+sum1+sum2};
            }
        }
        if(c=='-' || c=='?') {
            for(int j=-N; j<=N; ++j) {
