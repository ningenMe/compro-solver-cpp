...
void solve() {
    ...
    auto tree = StaticTree<StaticTreeOperator<int>>::builder(g).root(0).child().build();
    vector<multiset<int>> dp(N);
    for(int pa: tree.order) {
        dp[pa].insert(A[pa]);
        for(auto [ch,_]: tree.child[pa]) {
            for(auto a: dp[ch]) dp[pa].insert(a);
        }
    }
    int ans = 0;
    for(int i=0;i<N;++i) {
        ...
        int flg = 0;
        int sum = accumulate(cnt.begin(), cnt.begin()+K, 0);
        if(sum+y >= K && y<=1 && cnt[K]==0) flg=1;
        ans |= flg;
    }
    cout << (ans ? "Alice" : "Bob") << "\n";
}
