...
vector<int> ro(vector<int> v, int si, int sj) {
    vector<int> a,b,c;
    for(int i=0; i < v.size(); ++i) {
        if(i==si || i==si+1) a.push_back(v[i]);
        else b.push_back(v[i]);
    }
    if(sj==-1) c.push_back(a[0]), c.push_back(a[1]);
    for(int i=0;i<b.size(); ++i) {
        c.push_back(b[i]);
        if(i==sj) c.push_back(a[0]), c.push_back(a[1]);
    }
    return c;
}
...
int main() {
    ...
    RangeInversionQuery<int> riq(A);
    corner(riq.fold(0,N)%2==1, "No");
    
    vector<pair<int,int>> ans;
    for(int i=0;i<N;++i) {
        if(i == A[i]) continue;
        int k = -1;
        for(int j=0;j<N;++j) if(A[j]==i) k = j;
