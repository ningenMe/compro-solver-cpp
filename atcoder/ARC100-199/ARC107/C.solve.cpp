    ...
    modint ans=1;
    {
        ...
        for(int j=0; j < N; ++j) {
            for(int k=j+1; k<N; ++k) {
                int flg = 1;
                for(int i=0;i<N;++i) {
                    if(A[i][j]+A[i][k]>K) flg = 0;
                }
                if(flg) uf.merge(j,k);
            }
        }
        vector<int> cnt(N,0);
        for(int j=0;j<N;++j) cnt[uf[j]]++;
        for(int j=0;j<N;++j) ans *= CM.factorial(cnt[j]);
    }
    {
        UnionFindTree uf(N);
        for(int i=0; i < N; ++i) {
            for(int k=i+1; k<N; ++k) {
                int flg = 1;
                for(int j=0;j<N;++j) {
                    if(A[i][j]+A[k][j]>K) flg = 0;
                }
                if(flg) uf.merge(i,k);
            }
        }
        vector<int> cnt(N,0);
        for(int i=0;i<N;++i) cnt[uf[i]]++;
        for(int i=0;i<N;++i) ans *= CM.factorial(cnt[i]);
    }
    cout << ans << endl;
    return 0;
}
