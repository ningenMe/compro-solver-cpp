...
int main() {
    ...
    auto dp = multivector(K+1,N+1,N+1,HIGHINF);
    int init_r = uf.max(0);
    for(int i=0;i<=init_r+1;++i) dp[0][i][N]=0;
    for(int i=1;i<=K;++i) {
        for(int64 l=0;l<=N;++l) {
            for(int64 r=N; l<=r;--r) {
                //コストを使わない
                chmin(dp[i][l][r], dp[i-1][l][r]);

                //左側使う
                for(int64 k=1;l+k<=r;++k) {
                    if(l+k>N || !uf.connected(l,l+k-1)) break;
                    if(l+k<=r) chmin(dp[i][l+k][r], dp[i-1][l][r]+ (r-l)*C[i-1]);
                }

                //右側使う
                for(int64 k=1;l<=r-k;++k) {
                    if(0>r-k || !uf.connected(r-k,r-1)) break;
                    if(l<=r-k) chmin(dp[i][l][r-k], dp[i-1][l][r]+ k*C[i-1]);
                }
            }
        }
    }
    int64 ans = HIGHINF;
    for(int i=0;i<=N;++i) chmin(ans, dp[K][i][i]);
