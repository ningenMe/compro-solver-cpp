...
int main() {
    ...
    int64 X,Y,T; cin >> X >> Y >> T;
    auto vv = multivector(2,2,modint(0));
    vv[0] = {1,-1};
    vv[1] = {1,1};
    Matrix<modint,2,2> matrix(vv);
    matrix = matrix.pow(T);
    modint aX = matrix[0][0]*X + matrix[0][1]*Y;
    modint aY = matrix[1][0]*X + matrix[1][1]*Y;
    cout << aX << " " << aY << endl;
    return 0;
}
