...
int main() {
    ...
    auto Z = Zarts(A);
    auto B = multivector(N,K,0);
    for(int i=0;i<N;++i) for(int j=0;j<K;++j) B[i][j] = Z[i*K+j];
    int M = *max_element(ALL(Z)) + 1;
    auto dp = multivector(N,M,modint(0));
    {
        for(int j=0;j<K;++j) dp[0][B[0][j]]+=1;
        for(int j=1;j<M;++j) dp[0][j]+=dp[0][j-1];
    }
    for(int i=1;i<N;++i) {
        for(int j=0;j<K;++j) dp[i][B[i][j]]+=dp[i-1][B[i][j]];
