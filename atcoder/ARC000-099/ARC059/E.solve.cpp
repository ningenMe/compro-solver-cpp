...
int main() {
    ...
    int M = 400;
    vector<vector<Mint>> vv(C+1,vector<Mint>(M+1,1));
    vv[0][0]=0;
    for(int i=1;i<=C;++i) {
        for(int j=0;j<=M;++j) vv[i][j]=vv[i-1][j]*j;
    }
    for(int i=0;i<=C;++i) {
        for(int j=1;j<=M;++j) vv[i][j]+=vv[i][j-1];
    }
    vector<Fps> vf(N);
    for(int i=0;i<N;++i) {
        Fps f(C+1,0);
        for(int j=0;j<=C;++j) {
            f[j] = vv[j][B[i]] - vv[j][A[i]-1];
        }
        vf[i]=f;
    }
    cout << Fps::fold_all(vf,C+1)[C] << endl;
    return 0;
}
