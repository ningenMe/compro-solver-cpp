...
template<class T> struct MonoidRangeMaxPointUpdate {
    using TypeNode = T;
    inline static constexpr TypeNode unit_node = HIGHINF;
...
int main() {
    ...
    int N = 1<<20;
    SegmentTree<MonoidRangeMaxPointUpdate<int64>> seg(2*N,-1);
    int Q; read(Q);
    while(Q--) {
        int64 t,x; read(t), read(x);
        if(t==1) {
            int64 i = x%N;
            int64 j = seg.prefix_binary_search(i,2*N,-1) % N;
            seg.operate(j,x);
            seg.operate(j+N,x);
        }
        if(t==2) {
