...
int main() {
    ...
    corner(accumulate(ALL(D),0LL) != 2*(N-1), -1);
    UnionFindTree uf(N);
    for(int i=0;i<M;++i) {
        int u,v; read(u),read(v);
        u--,v--;
        corner(uf.connected(u,v), -1);
        uf.merge(u,v);
        D[u]--,D[v]--; 
    }
    ...
    for(int i=0;i<N;++i) {
        int r = uf[i];
        for(int j=0;j<D[i];++j) mv[r].push_back(i);
    }    
    priority_queue<pair<int,int>> pq;
    for(auto& [r,v]: mv) pq.emplace(v.size(),r);

    auto [sz_x, r_x] = pq.top(); pq.pop();
    auto ve_x = mv[r_x];
    vector<pair<int,int>> ans;
    while(pq.size()) {
        auto [sz_y, r_y] = pq.top(); pq.pop();
        auto ve_y = mv[r_y];
        int u=ve_x.back(), v = ve_y.back();
        
        ans.emplace_back(u+1,v+1);
        uf.merge(u,v);
        ve_x.pop_back();
        ve_y.pop_back();
        for(auto v: ve_y) ve_x.push_back(v);
    }
    corner(ve_x.size(),-1);
