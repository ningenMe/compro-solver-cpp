...
int main() {
    ...
    Mint ans=1;
    for(int i=0;i<N;++i) if(deg[i] == 0 && vst[i] == 0) ans*=0;
    for(int i=0;i<N;++i) if(!(deg[i] == 0 || deg[i] == 2)) ans*=0;
    UnionFindTree uf(N);
    for(int i=0;i<N;++i) if(deg[i] == 2) for(int j: edge[i]) uf.merge(i,j);
    set<int> st;
    for(int i=0;i<N;++i) if(uf.size(i) > 1) st.insert(uf[i]);
    ans *= Mint(2).pow(st.size());
    cout << ans << endl;
    return 0;
}
