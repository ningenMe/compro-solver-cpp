...
template<class T, class U> struct MonoidRangeFoldMinRangeOperateUpdate {
	using TypeNode = T;
	using TypeLazy = U;
	inline static constexpr TypeNode unit_node = {-1,-1};
	inline static constexpr TypeLazy unit_lazy = {-1,-1};
...
int main() {
    ...
    int N; read(N);
    string S; read(S);
    LazySplayTreeSequence<MonoidRangeFoldMinRangeOperateUpdate<pair<int,int>,pair<int,int>>> st1;
    st1.insert(0,{0,0});
    for(int i=0;i<N;++i) {
        auto [_,k] = st1.fold(0,i+1);
        int d = (S[i]=='L'?0:1);
        st1.insert(k+d,{i+1,k+d});
    }
    for(int i=0;i<=N;++i) cout << st1.get(i).first << " \n"[i==N]; 
    return 0;
}
