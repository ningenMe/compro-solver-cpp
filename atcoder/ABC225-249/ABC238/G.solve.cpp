...
template<class T> struct NodeSum {
    using TypeNode = T;
    inline static TypeNode unit_node = unordered_map<int,int>();
...
int main() {
    ...
    for(int i=0;i<N;++i) {
        int a; read(a);
        auto v = Prime::factor(a);
        for(int e:v) vm[i][e]++;
    }
    DisjointSparseTable<NodeSum<unordered_map<int,int>>> dst(vm);
    while(Q--) {
        int l,r; read(l),read(r);
        l--;
        auto mp = dst.fold(l,r);
        int flg = 1;
        for(auto [_,val]: mp) if(val % 3) flg = 0;
        cout << (flg ? "Yes" : "No") << "\n";
    }
    return 0;
}
