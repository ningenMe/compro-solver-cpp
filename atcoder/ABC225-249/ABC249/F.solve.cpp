...
int main() {
    ...
    vector<int64> T(N+1),Y(N+1);
    T[0]=1,Y[0]=0;
    for(int i=1;i<=N;++i) read(T[i]),read(Y[i]);

    RandomizedBinarySearchTreeSet<MonoidRangeSumPointAdd<int64>> st_l,st_r;

    int64 ans=-HIGHINF;
    for(int i=N; 0 <=i && 0<=K; --i) {
        if(T[i]==1) {
            int64 sum = 0;
            sum += st_r.fold(0,st_r.size());
            if(st_l.size() > K) sum += st_l.fold(K,st_l.size());
            chmax(ans, Y[i]+sum);
            K--;
        }
        else {
            if(Y[i]>=0) st_r.insert(Y[i]);
            else st_l.insert(Y[i]);
        }
    }
    cout << ans << endl;
    return 0;
