...
int main() {
    ...
    int N,M,B,W; read(N),read(M),read(B),read(W);
    auto dp = multivector(N+1,M+1,Mint(0));
    CombinationMod<MOD_998244353> cm(N*M+10);
    Mint ans=0;
    for(int i=1;i<=N;++i) {
        for(int j=1;j<=M;++j) {
            int s=i*j;
            int t=(N-i)*(M-j);
            if(s<B || t<W) continue;
            ans+=cm.binom(N,i)*cm.binom(M,j)*cm.binom(s,B)*cm.binom(t,W);
        }
    }
    cout << ans << endl;
    return 0;
