...
template<class T> struct NodeMinPointUpdate {
    using TypeNode = T;
    inline static constexpr TypeNode unit_node = {-1,1234567};
...
int main() {
    ...
    int P_MAX=200010;
    SegmentTree<NodeMinPointUpdate<pair<int,int>>> seg(P_MAX);
    for(int i=0; i < P_MAX; ++i) seg.update(i,{-1,i});
    int N; cin >> N;
    while(N--) {
        int p; cin >> p;
        seg.update(p,{p,p});
        cout << seg.get(0,P_MAX).second << endl;
    }
    return 0;
}
