...
int main() {
    ...
    vector<UnionFindTree> A(H,UnionFindTree(W)),B(W,UnionFindTree(H));
    for(int i=0;i<H;++i) {
        for(int j=0;j+1<W;++j) if(S[i][j]==S[i][j+1]) A[i].merge(j,j+1);
    }
    for(int j=0;j<W;++j) {
        for(int i=0;i+1<H;++i) if(S[i][j]==S[i+1][j]) B[j].merge(i,i+1);
    }
    int K=0;
    for(int i=0;i<H;++i) {
        for(int j=0;j<W;++j) {
    ...
    modint ans = 0;
    modint cnt = modint(2).pow(K);
    for(int i=0;i<H;++i) {
        for(int j=0;j<W;++j) {
            if(S[i][j]!='.') continue;
            int M = A[i].size(j) + B[j].size(i) - 1;
            ans += cnt - modint(2).pow(K-M);
        }
    }
    cout << ans << endl;
    return 0;
