...
template<class T> struct FormalPowerSeries : public vector<T> {
    ...
    template<int prime> inline vector<ModInt<prime>> convolution_friendlymod(const vector<Mint>& a,const vector<Mint>& b){
    ...
    inline vector<ModInt<998244353>> convolution(const vector<ModInt<998244353>>& g,const vector<ModInt<998244353>>& h){return convolution_friendlymod<998244353>(g,h);}
...
public:
    ...
    Mint substitute(Mint a) {
        Mint base = 1,ret = 0;
        for(int i = 0; i < this->size(); base*= a,i++) ret += (*this)[i]*base;
        return ret;
    }
    ...
    for(int i=0;i+1<X.size();++i) {
        ...
        while(v.size()>1) {
            ...
            for(int i=0;i<v.size();i+=2) {
                if(i+1<v.size()) tmp.push_back(v[i]*v[i+1]);
                else tmp.push_back(v[i]);
            }
            v=tmp;
        }
        fps f = v.front();
        f = (f.diff()*fps({0,1})).intg();
        E += f.substitute(xr) - f.substitute(xl);
    }
    for(int i=1;i<=N+1;++i) E *= i;
    for(int i=0;i<N;++i) E *= (R[i]-L[i]);
    cout << E << endl;
    return 0;
}
