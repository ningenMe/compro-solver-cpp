...
int main() {
    SPEED
    ll N,X,M=100000; cin >> N >> X;
    vector<vector<ll>> B(M+1);
    vector<ll> T(N),A(N);
    for(int i = 0; i < N; ++i) cin >> T[i];
    for(int i = 0; i < N; ++i) cin >> A[i];
    for(int i = 0; i < N; ++i) B[T[i]].push_back(A[i]);
    int ok = M+1, ng=0, md, flg = 0;
    while(ok-ng>1) {
        md=(ok+ng)/2;
        priority_queue_reverse<ll> pq;
        ...
        SegmentTree<NodeSumPointAdd<ll>> seg(md);
        int cnt=0;
        while(pq2.size()) {
            auto p=pq2.top();
            pq2.pop();
            ll a=p.first;
            int t=p.second;
            if(cnt+pq.size()==md && pq.size() && a <= pq.top()) continue;
            if(seg.get(0,t)==t) continue;
            int ok2=0,ng2=t,md2;
            while(ng2-ok2>1){
                md2=(ok2+ng2)/2;
                (seg.get(md2,t)<(t-md2)?ok2:ng2)=md2;
            }
            seg.update(ok2,1);
            sum += a;
            cnt++;
            while(md-cnt<pq.size()) {
                sum -= pq.top();
                pq.pop();
            }
        }
