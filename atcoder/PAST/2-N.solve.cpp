...
int main() {
	...
	int N,Q; cin >> N >> Q;
	vector<ll> A(4*N+2*Q),C(N);
	for(int i = 0; i < N; ++i) {
		ll x,y,d; cin >> x >> y >> d >> C[i];
		A[4*i+0] = x;
		A[4*i+1] = x+d;
		A[4*i+2] = y;
		A[4*i+3] = y+d;
	...
	vector<vector<int>> area_xmin(M),area_xmax(M),point_x(M);
	for(int i = 0; i < N; ++i) {
		int xmin = Z[4*i+0],xmax = Z[4*i+1];
		area_xmin[xmin].push_back(i);
		area_xmax[xmax].push_back(i);
	}
	for(int i = 0; i < Q; ++i) {
		int x = Z[4*N+2*i+0];
		point_x[x].push_back(i);
	}
	LazySegmentTree<NodeSumRangeAdd<ll,ll>> seg(M,0);
	vector<ll> ans(Q);
	for(int i=0;i<M;++i) {
		//面積加算
		for(int j:area_xmin[i]) {
			int ymin = Z[4*j+2],ymax = Z[4*j+3];
			seg.update(ymin,ymax+1,C[j]);
		}
		for(int j:point_x[i]) {
			int y = Z[4*N+2*j+1];
			ans[j] = seg.get(y,y+1);
