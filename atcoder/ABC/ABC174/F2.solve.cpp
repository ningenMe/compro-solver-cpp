...
int main() {
	...
	vector<vector<int>> query(N+1),range(N+1);
    ...
    SegmentTree<NodeSumPointAdd<int>> seg(N+1);
    vector<int> ans(Q);
    for(int l = 1; l <= N; ++l){
        for(int r:range[l]) seg.update(r,1);
        for(int j:query[l]) ans[j]=N - seg.get(R[j],N+1);
    }
