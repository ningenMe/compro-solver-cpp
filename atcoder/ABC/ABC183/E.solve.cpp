...
int main() {
    ...
    auto dp   = multivector(H+1,W+1,modint(0));
    auto dprr = multivector(H+1,W+1,modint(0));
    auto dpdd = multivector(H+1,W+1,modint(0));
    auto dpdr = multivector(H+1,W+1,modint(0));
    ...
    for(int i=0;i<H;++i) {
        for(int j=0;j<W;++j) {
            ...
            dprr[i][j+1]  +=dprr[i][j];
            dpdd[i+1][j]  +=dpdd[i][j];
            dpdr[i+1][j+1]+=dpdr[i][j];

            dp[i][j] += dprr[i][j]+dpdd[i][j]+dpdr[i][j];
