...
int main() {
    ...
    int K; string S; read(K); read(S);
	int N=S.size();
	//https://maspypy.com/atcoder-%e5%8f%82%e5%8a%a0%e6%84%9f%e6%83%b3-2020-06-21abc-17#toc4
	//f=1+25x+25^2x^2+25^3x^3+...
	//f= 1 / (1-25x)
	//同様に
	//g= 1 / (1-26x)
	//[x^K] f^N g を求める
	//fi=1-25xとして
	//f^N = (1 / fi)^N = (1 / (1-25x))^N = (1-25x)^(-N) = Σ (n+N-1)_C_(N-1) (25x)^n
	using Fps = FormalPowerSeries<MOD_1000000007>;
	CombinationMod<MOD_1000000007> CM(2*N+K);
	Fps f(N+K,0), g = {1,-26};
	f[0]=1;
	ModInt<MOD_1000000007> co=1;
	for(int i=1;i<N+K;++i) {
		co *= 25;
		f[i] = co * CM.binom(i+N-1,N-1);
	}
	cout << Fps::nth_term(K,f,g) << endl;
    return 0;
}
