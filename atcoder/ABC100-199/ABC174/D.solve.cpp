...
int main() {
    ...
    DisjointSparseTable<NodeSum<int>> rsum(r),wsum(w);
    chmin(ans,rsum.get(0,N));
    chmin(ans,wsum.get(0,N));
    for(int i = 0; i+1 < N; ++i){
        int cr=wsum.get(0,i+1);
        int cw=rsum.get(i+1,N);
        int m = min(cr,cw);
        cr -= m;
        cw -= m;
        chmin(ans,m+max(cr,cw));
    }
    cout << ans << endl;
    return 0;
}
