...
int main() {
    ...
    int H,W; cin >> H >> W;
    LazySegmentTree<NodeMinRangeArithmeticUpdate<int,pair<int,int>>> seg(W+1);
    int inf = 1234567;
    seg.update(0,1,{inf,0});
    for(int j=1; j <= W; ++j) seg.update(j,j+1,{0,j});
    vector<int> ans(H);
    for(int i = 0; i < H; ++i) {
        int l,r; cin >> l >> r; r++;
        int cnt = seg.get(l-1,l);
        seg.update(l,r,{cnt,l-1});
        ans[i] = seg.get(1,W+1) + i+1;
