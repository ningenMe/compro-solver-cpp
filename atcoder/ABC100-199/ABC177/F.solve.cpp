...
//node:総和　lazy:更新
template<class T, class U> struct NodeMinRangeUpdate {
	using TypeNode = T;
	using TypeLazy = U;
	inline static constexpr TypeNode unit_node = {LOWINF,LOWINF};
	...
	inline static constexpr TypeNode func_merge(TypeNode l,TypeLazy r,int len){return r.first!=-2000?r:l;}
...
int main() {
    ...
    int H,W; cin >> H >> W;
    LazySegmentTree<NodeMinRangeUpdate<pair<ll,ll>,pair<ll,ll>>> seg(W);
    for(int j=0; j < W; ++j) seg.update(j,j+1,{1,j});
    vector<ll> ans(H);
    for(int i = 0; i < H; ++i) {
        ll l,r; cin >> l >> r;
        l--;
        if(r!=W && seg.get(0,r).first!=LOWINF && seg.get(r,r+1).first==LOWINF) {
            ll ok=0,ng=r,md;
            while(ng-ok>1) {
                md=(ok+ng)/2;
                (seg.get(md,r).first!=LOWINF?ok:ng)=md;
            }
            auto p = seg.get(ok,ok+1);
            seg.update(r,r+1,{p.first+(r-p.second),r});
        }
        seg.update(l,r,{LOWINF,LOWINF});
        ans[i] = seg.get(0,W).first + i;
        if(ans[i]>=LOWINF) ans[i] = -1;
    }
    for(int i = 0; i < H; ++i) {
        cout << ans[i] << endl;
    }
