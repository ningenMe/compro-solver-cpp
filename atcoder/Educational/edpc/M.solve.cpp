...
public:
    ...
    inline long long binom(int n, int k) const {
        return ((n < 0 || k < 0 || n < k) ? 0 : fac[n] * (finv[k] * finv[n - k] % mod) % mod);
    }
    inline long long factorial(int n) const {
        return fac[n];
    }
...
public:
    ...
    //(1-rx)^(-n) = Σ (i+n-1)_C_(n-1) (rx)^i を計算して返してくれる
...
int main() {
    ...
    using Sfps = SparseFormalPowerSeries<MOD_1000000007>;
    Fps f(1,1);
    for(int i=0;i<N;++i) {
        int a; read(a);
        // g = 1 + x + x^2 + ... + x^a
        // g = (1-x^(a+1)) / (1 - x)
        // f = (1-x^(a1+1))*(1-x^(a2+1))*...*(1-x^(an+1)) * Σ (s+n-1)_C_(n-1) (x)^s
        Sfps g;
        g.update(0,1);
        g.update(a+1,-1);
        f = Fps::mul(f,g).prefix(K+1);
    }
    CombinationMod<MOD_1000000007> cm(N+K);
    Fps h = Fps::negative_binomial_theorem(1,N,K+1,cm);
    cout << Fps::mul(f,h)[K] << endl;
    return 0;
}
