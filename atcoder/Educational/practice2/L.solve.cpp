...
template <class T>ostream &operator<<(ostream &o, const array<T, 3>&obj) {o << "{"; for (int i = 0; i < (int)obj.size(); ++i)o << (i > 0 ? ", " : "") << obj[i]; o << "}"; return o;}
...
//node:総和　lazy:更新
template<class T, class U> struct NodeInversionRangeXor {
	using TypeNode = T;
	using TypeLazy = U;
	inline static constexpr TypeNode unit_node = array<ll,3>({0,0,0});
	inline static constexpr TypeLazy unit_lazy = 0;
	inline static constexpr TypeNode func_node(const TypeNode l,const TypeNode r){return {l[0]+r[0],l[1]+r[1],l[1]*r[0]+l[2]+r[2]};}
	inline static constexpr TypeLazy func_lazy(const TypeLazy l,const TypeLazy r){return l^r;}
	inline static constexpr TypeNode func_merge(const TypeNode l,const TypeLazy r,const int len){return (r?array<ll,3>({l[1],l[0],l[0]*l[1]-l[2]}):l);}
...
int main() {
    ...
    LazySegmentTree<NodeInversionRangeXor<array<ll,3>,int>> seg(v);
