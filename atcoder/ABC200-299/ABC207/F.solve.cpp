...
int main() {
    ...
    auto tree = StaticTree<StaticTreeOperator<int>>::builder(g).root(0).child().subtree_size().build();
    //dp[i][j][k][l] i番目の頂点に、j:i自身に警備が配置されてるかどうか、k:i自身が警備対象にすでにカウントされてるかどうか、部分木にl個の頂点が警備されてる 時の個数
    auto dp = multivector(N,2,2,2,Mint(0));
    auto bp = multivector(2,2,2,Mint(0));

    for(int pa: tree.order) {
        dp[pa][0][0][0]=1;
        dp[pa][1][1][1]=1;
        int sz_pa=1;
        for(auto [ch,_]: tree.child[pa]) {
            int sz_ch = tree.subtree_size[ch];
            auto tp = bp;
            for(int i=0;i<2; ++i) for(int j=0;j<2; ++j) tp[i][j].resize(sz_pa+sz_ch+1, 0);
            for(int i=0; i <= sz_pa; ++i) {
                for(int j=0; j<= sz_ch; ++j) {
                    if(i+j  <=sz_pa+sz_ch) tp[0][0][i+j  ]  += dp[pa][0][0][i] * dp[ch][0][0][j];
                    if(i+j  <=sz_pa+sz_ch) tp[0][0][i+j  ]  += dp[pa][0][0][i] * dp[ch][0][1][j];
                    if(i+j+1<=sz_pa+sz_ch) tp[0][1][i+j+1]  += dp[pa][0][0][i] * dp[ch][1][0][j];
                    if(i+j+1<=sz_pa+sz_ch) tp[0][1][i+j+1]  += dp[pa][0][0][i] * dp[ch][1][1][j];

                    if(i+j  <=sz_pa+sz_ch) tp[0][1][i+j  ]  += dp[pa][0][1][i] * dp[ch][0][0][j];
                    if(i+j  <=sz_pa+sz_ch) tp[0][1][i+j  ]  += dp[pa][0][1][i] * dp[ch][0][1][j];
                    if(i+j  <=sz_pa+sz_ch) tp[0][1][i+j  ]  += dp[pa][0][1][i] * dp[ch][1][0][j];
                    if(i+j  <=sz_pa+sz_ch) tp[0][1][i+j  ]  += dp[pa][0][1][i] * dp[ch][1][1][j];

                    if(i+j+2<=sz_pa+sz_ch) tp[1][1][i+j+2]  += dp[pa][1][0][i] * dp[ch][0][0][j];
                    if(i+j+1<=sz_pa+sz_ch) tp[1][1][i+j+1]  += dp[pa][1][0][i] * dp[ch][0][1][j];
                    if(i+j+2<=sz_pa+sz_ch) tp[1][1][i+j+2]  += dp[pa][1][0][i] * dp[ch][1][0][j];
                    if(i+j+1<=sz_pa+sz_ch) tp[1][1][i+j+1]  += dp[pa][1][0][i] * dp[ch][1][1][j];

                    if(i+j+1<=sz_pa+sz_ch) tp[1][1][i+j+1]  += dp[pa][1][1][i] * dp[ch][0][0][j];
                    if(i+j  <=sz_pa+sz_ch) tp[1][1][i+j  ]  += dp[pa][1][1][i] * dp[ch][0][1][j];
                    if(i+j+1<=sz_pa+sz_ch) tp[1][1][i+j+1]  += dp[pa][1][1][i] * dp[ch][1][0][j];
                    if(i+j  <=sz_pa+sz_ch) tp[1][1][i+j  ]  += dp[pa][1][1][i] * dp[ch][1][1][j];
                }
            }
            swap(tp,dp[pa]);
            sz_pa += sz_ch;
