...
int main() {
    ...
    vector<int64> v;
    for(int64 e:st) v.push_back(e);
    int M = v.size();

    SegmentTree<MonoidRangeSumPointAdd<int64>> seg0(M,0),seg1(M,0);
    for(int i=0;i<M;++i) {
        seg0.operate(i,mp0[v[i]]);
        seg1.operate(i,mp1[v[i]]);
    }
    int64 ans = 0;
    for(int i=0;i<M;++i) {
        chmax(ans,seg0.fold(0,i) + seg1.fold(i+1,M));
    }
    cout << ans << endl;
