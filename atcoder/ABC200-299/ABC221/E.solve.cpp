...
template<class T, class U> struct MonoidRangeSumRangeUpdate {
	...
	inline static constexpr TypeLazy unit_lazy = {0,1};
	inline static constexpr TypeNode func_fold(TypeNode l,TypeNode r){return l+r;}
	inline static constexpr TypeLazy func_lazy(TypeLazy old_lazy,TypeLazy new_lazy){return {new_lazy.first,old_lazy.second * new_lazy.second};}
	inline static constexpr TypeNode func_operate(TypeNode node,TypeLazy lazy,int l, int r){return lazy.first != 0 ? node + (r-l) : node*lazy.second;}
...
int main() {
    ...
    LazySegmentTree<MonoidRangeSumRangeUpdate<modint, pair<int,modint>>> seg(N);
    modint ans = 0;
    for(int i = 0; i < N; ++i) {
        int j = idx[i];
