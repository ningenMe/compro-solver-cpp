...
int main() { 
    ...
    auto tree = Tree<TreeOperator<int64>>::builder(g).root(0).build();
    auto vv = multivector(60,2,0);
    vector<modint> cnt(60,0);
    for(int64 d:tree.edge_dist) {
        for(int64 b=0;b<60;++b) {
            int dd = (d>>b&1LL);
            cnt[b] += vv[b][dd^1];
            vv[b][dd]+=1;
        }
