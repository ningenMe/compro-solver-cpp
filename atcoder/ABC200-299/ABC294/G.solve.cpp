...
int main() {
    ...
    int64 N; cin >> N;
    vector<pair<pair<size_t,size_t>,int64>> vp(N-1);
    Graph<int64> g(N);
    for(int i=0;i+1<N;++i){
		int u,v;
        int64 w; 
    ...
    auto tree = Tree<TreeOperator<int64>>::builder(g).root(0).child().subtree_size().parent().heavy_light_decomposition().build();
    SegmentTree<MonoidRangeSumPointAdd<int64>> seg(N);


    for(int i=0;i+1<N;++i) {
		int l = vp[i].first.first, r = vp[i].first.second;
        l = tree.hld[l],r = tree.hld[r];
        seg.operate(max(l,r),vp[i].second);
    }    
    ...
    while(Q--) {
        ...
        if(x == 1){
			...
			int l = vp[i].first.first, r = vp[i].first.second;
            l = tree.hld[l],r = tree.hld[r];
            seg.operate(max(l,r),w);
        }
        else{
            int l,r; cin >> l >> r;
            l--,r--;
            if(l>r) swap(l,r);
            auto tmp = tree.edge_set_on_path(l,r);
            int64 ans = 0;
            for(auto& p:tmp){
