...
template<class T> struct NodeMaxPointUpdate {
	using TypeNode = T;
	inline static constexpr TypeNode unit_node = {0,1};
...
int main() {
    ...
    auto Z = Zarts(A);
    vector<vector<pair<int,int>>> redge(2*N);
    for(int i = 0; i < N; ++i) {
        int l = Z[i],r = Z[i+N];
        redge[r].push_back({l,i});
    }
    SegmentTree<NodeMaxPointUpdate<pair<int,int>>> dp(2*N);
    for(int r = 2*N-1; 0 < r; --r) {
        for(auto p:redge[r]) {
            int l = p.first, i = p.second;
            auto s = dp.get(r,2*N);
            auto t = make_pair(s.first+1,-i);
            auto u = dp.get(l,l+1);
            if(u<t) dp.update(l,t);
        }
    }
    vector<int> ans;
    for(int l = 0;;) {
