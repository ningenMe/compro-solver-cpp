...
template<class T, class U> struct NodeMinRangeUpdate {
	...
	inline static constexpr TypeLazy unit_lazy = 1234567890;
	...
	inline static constexpr TypeNode func_merge(TypeNode l,TypeLazy r,int len){return r!=unit_lazy?r:l;}
	inline static constexpr TypeNode func_check(TypeLazy l,TypeLazy r){return l<r;}
...
int main() {
	SPEED
	int N; cin >> N;
	vector<ll> C(N),A(N),G(N,0);
	for(int i = 1; i < N; ++i) cin >> C[i] >> A[i];
	LazySegmentTree<NodeMinRangeUpdate<ll,ll>> seg(N,-1);
	seg.update(0,1,0);
	int ans=0;
	for(int i = 1; i < N; ++i) {
		ll l = i-C[i];
		G[i] = seg.prefix_binary_search(l);
		seg.update(G[i],G[i]+1,i);
	}
	for(int i = 1; i < N; ++i) if(A[i]%2) ans ^= G[i];
