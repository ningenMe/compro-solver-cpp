...
int main() {
	...
	const int M=2000001;
	vector<double> p(M,0);
	for(int i = 1; i < M; ++i) p[i]=log(i);
	for(int i = 1; i < M; ++i) p[i]+=p[i-1];
	SegmentTree<NodeSumPointUpdate<double>> seg(N+2);
	for(int i = 0; i+1 <= N+1; ++i) {
		int dx=abs(x[i]-x[i+1]);
		int dy=abs(y[i]-y[i+1]);
		seg.update(i,p[dx+dy]-p[dx]-p[dy]);
	}
	int Q; cin >> Q;
	while(Q--){
		int t; cin >> t;
		if(t==1){
			...
			double a=seg.get(l,r),b=seg.get(s,t);
