...
int main() {
    ...
    vector<int64> P(N),L(M),D(M),A(M),B(M);
    for(int i=0;i<N;++i) read(P[i]);
    for(int i=0;i<M;++i) read(L[i]);
    for(int i=0;i<M;++i) read(D[i]);
    sort(ALL(P));
    vector<int> idx(M);
    iota(ALL(idx),0);
    sort(ALL(idx),[&](int l, int r) {
        return (L[l]==L[r] ? D[l] < D[r] : L[l] < L[r]);
    });
    for(int i=0;i<M;++i) {
        A[i] = L[idx[i]];
        B[i] = D[idx[i]];
    }
    SegmentTree<MonoidRangeMaxPointUpdate<pair<int64,int64>>> seg(M);
    for(int i=0;i<M;++i) {
        seg.operate(i,{B[i],i});
    }
    int64 ans = 0;
    for(int i = 0; i < N; ++i) {
        auto itr = upper_bound(ALL(A),P[i]);
        if(itr == A.begin()) {
            ans += P[i];
            continue;
        }
        int r = M;
        if(itr != A.end()) r = (itr - A.begin());

        auto [b,j] = seg.fold(0, r);
        if(b == -1) {
            ans += P[i];
            continue;
        }
