...
int main() {
    ...
    for(int i=0;i<M;++i) {
        int64 u,v,w; read(u), read(v), read(w);
        u--,v--;
        edge[u].push_back({v,w});
        edge[v].push_back({u,w});
    }
    int K; read(K);
    priority_queue_reverse<tuple<int64,int64,int>> pq;
    vector<pair<int64,int64>> dp(N,{LOWINF,0});
    for(int i=0;i<K;++i) {
        int a; read(a); a--;
        pq.push({0,0,a});
        dp[a]={0,0};
    ...
    int D; read(D);
    SegmentTree<MonoidRangeMaxPointUpdate<int64>> seg(D+1,0);
    for(int i=1;i<=D;++i) {
        int64 x; read(x);
        seg.operate(i,x);
    }
    ...
    while(pq.size()) {
        auto [day,cost,from] = pq.top(); pq.pop();
        if(dp[from]<make_pair(day,cost)) continue;

        //そのまま進む
        for(auto [to,w]: edge[from]) {
            if(cost + w <= 0) {
                if(dp[to] > make_pair(day,cost+w)) {
                    pq.push({day,cost+w,to});
                    dp[to] = {day,cost+w};
                }
            }
            else {
                int64 n_day = seg.prefix_binary_search(day+1, D+1, w);
                if(n_day > D) continue;
                int64 x = seg.fold(n_day,n_day+1);
                if (dp[to] > make_pair(n_day, -x+w)) {
                    pq.push({n_day, -x+w, to});
                    dp[to] = {n_day, -x+w};
                }
            }
        }
