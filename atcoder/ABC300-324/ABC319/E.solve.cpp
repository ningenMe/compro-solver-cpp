...
int main() {
    ...
    int64 L = 1;
    for(int i=1;i<=8;++i) L = Prime::lcm(L,i);
    vector<int64> dp(L,LOWINF),tp(L);
    for(int i=0;i<L;++i) dp[i]=i;
    for(int i=1;i<N;++i) {
        int64 p,t; read(p),read(t);
        for(int j=0;j<L;++j) tp[j]=LOWINF;
        for(int j=0;j<L;++j) {
            int64 q = dp[j]%p;
            chmin(tp[j],dp[j]+t+(q==0?0:p-q));
        }
        dp=tp;
    }
    int Q; read(Q);
