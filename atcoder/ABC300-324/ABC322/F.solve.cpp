 ...
 */
template<class T, class U> struct MonoidRangeFoldSumRangeOperateFlip {
    using TypeNode = T;
    using TypeLazy = U;
    ...
    inline static constexpr TypeNode func_fold(TypeNode l,TypeNode r){
        ...
        int v_1_prefix = l[0] + (l[0] == l[6] ? r[0] : 0);
        int v_1_suffix = r[1] + (r[1] == r[6] ? l[1] : 0);
        int v_0_prefix = l[3] + (l[3] == l[6] ? r[3] : 0);
        int v_0_suffix = r[4] + (r[4] == r[6] ? l[4] : 0);
        return {
            v_1_prefix,
            v_1_suffix,
            max({l[2],r[2],v_1_prefix,v_1_suffix,l[1]+r[0]}),
            v_0_prefix,
            v_0_suffix,
            max({l[5],r[5],v_0_prefix,v_0_suffix,l[4]+r[3]}),
            l[6]+r[6]
        };
    }
    ...
    inline static constexpr TypeNode func_operate(TypeNode node,TypeLazy lazy,int l, int r){
        if(lazy == 0) return node;
        int n = 3;
        for(int i=0;i<n;++i) swap(node[i],node[i+n]);
        return node;
    }
...
int main() {
    ...
    LazySegmentTree<MonoidRangeFoldSumRangeOperateFlip<Node,int>> seg(A);
