...
int main() {
    ...
    pair<int64,int64> inf = {LOWINF,LOWINF};

    vector<pair<int64,int64>> dp(N,inf);
    ...
    for(int i=0;i<M;++i) {
        ...
        a--,b--;
        edge[a].emplace_back(b,x,y);
        edge[b].emplace_back(a,-x,-y);
    }
    queue<int> q;
    dp[0]={0,0};
    q.push(0);
    while(q.size()) {
        ...
        for(auto [to,x,y]: edge[from]) {
            if(dp[to]!=inf) continue;
            dp[to] = {dp[from].first+x,dp[from].second+y};
            q.push(to);
        }
    }
    for(int i=0;i<N;++i) {
