...
int main() {
    ...
    auto tree = StaticTree<StaticTreeOperator<int64>>::builder(g).root(0).child().build();

    vector<int64> chdp(N,0),padp(N,0),ans(N,0);
    //木dp
    for(int pa: tree.order) {
        for(auto [ch,w]: tree.child[pa]) {
            chmax(chdp[pa],chdp[ch]+w);
            chmax(chdp[pa],A[ch]+w);
        }
    }
    SegmentTree<MonoidRangeMaxPointAdd<int64>> seg(N,0);
    for(int pa: tree.reorder) {
        chmax(ans[pa], chdp[pa]);
        if(pa) chmax(ans[pa], padp[pa]);

        int M = tree.child[pa].size();

        for(int i=0;i<M;++i) {
            auto [ch,w] = tree.child[pa][i];
            seg.operate(i,max(chdp[ch]+w,A[ch]+w));
        }
        seg.operate(M,padp[pa]);

        for(int i=0;i<M;++i) {
            auto [ch,w] = tree.child[pa][i];
            seg.operate(i,-max(chdp[ch]+w,A[ch]+w));
            chmax(padp[ch],seg.fold(0,N)+w);
            chmax(padp[ch],A[pa]+w);
            seg.operate(i,max(chdp[ch]+w,A[ch]+w));
        }
