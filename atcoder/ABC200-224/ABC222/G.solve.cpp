...
int main() {
    ...
    //          = 10 (an+ 2/10 + 2/90)
    //          = 10 (an+ 2/9)
    //a_n + 2/9 = (2+2/9) * 10 ^ (n-1)
    //a_n = (2 + 2/9) * 10^(n-1) - 2/9 = (18 + 2) / 9 * 10^(n-1) - 2/9 = (20 * 10^(n-1)) / 9 - 2/9 = (2 * 10^n - 2) / 9 = 2*(10^n-1)/9
    // (2/9)*(10^n-1) = 0 % K になる n
    int T; read(T);
    while(T--) {
        int64 K; read(K);
        int64 g = Prime::gcd(2,K);
        K = K * 9 / g;
        int64 ans=Prime::discrete_logarithm(10,1,K,0);
        cout << ans << "\n";
    }
    return 0;
}
