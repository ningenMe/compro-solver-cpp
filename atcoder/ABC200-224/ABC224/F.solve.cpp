...
int main() {
    ...
    //cdp[1]=(cdp[0])
    //sdp[1]=(sdp[0]+cdp[0]*a[1])

    //cdp[2]=(cdp[0]+cdp[1])
    //sdp[2]=(sdp[0]+cdp[0]*(a[1]*10+a[2]))+(sdp[1]+cdp[1]*a[2])

    //dp[2]=(dp[1]+a[2])+(dp[0]+a[1]*10+a[2])
    //dp[3]=(dp[2]+a[3])+(dp[1]+a[2]*10+a[3])+(dp[0]+a[1]*10^2+a[2]*10+a[3])

    //dp[N]=dp[0]+...+dp[N-1] + a[N]*N*10^{0} + a[N-1]*(N-1)*10^{1} + a[N-2]*(N-2)*10^{2}

    S = "0" + S;
    int N = S.size();
    using Mint = ModInt<MOD_998244353>;
    SegmentTree<MonoidRangeSumPointAdd<Mint>> sdp(N,0),cdp(N,0);
    cdp.operate(0,1);
    Mint coe=0;
    for(int i=1;i<N;++i) {
        Mint c = cdp.fold(0,i);
        coe *= 10;
        coe += Mint(S[i]-'0') * c;
        Mint s = sdp.fold(0,i) + coe;
        sdp.operate(i,s);
        cdp.operate(i,c);
    }
    cout << sdp.fold(N-1,N) << endl;
    return 0;
}
