...
template<class T, class U> struct MonoidRangeFoldMinRangeOperateUpdate {
    ...
    inline static constexpr TypeLazy unit_lazy = LOWINF;
...
int main() {
    ...
    int N; read(N);
    LazySegmentTree<MonoidRangeFoldMinRangeOperateUpdate<int64,int64>> seg(N);
    for(int i=0;i<N;++i) {
        int a; read(a);
        seg.operate(i,i+1,a);
    }
    ...
    while(Q--) {
        int q; read(q);
        if(q==1) {
            int64 x; read(x); seg.operate(0,N,x);
        }
        if(q==2) {
            int i; read(i); --i;
            int64 x; read(x);
