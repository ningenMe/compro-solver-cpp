...
int main() {
    ...
    for(int i=0;i<N;++i) seg.update(i,max(A[i+1]-B[i],0LL));
    unordered_map<ll,ll> mp;
    {
        if(!mp.count(A[0])) mp[A[0]]=HIGHINF;
        chmin(mp[A[0]],seg.get(0,N));
    }
    for(int i=0;i<N;++i) {
        seg.update(i,max(A[i]-B[i],0LL));
        if(!mp.count(A[i+1])) mp[A[i+1]]=HIGHINF;
        chmin(mp[A[i+1]],seg.get(0,N));
    }
    for(int i=0;i<=N;++i) {
        cout << mp[C[i]] << " \n"[i==N];
    }
